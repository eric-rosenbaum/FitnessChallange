...
import type { PunishmentProgress, PunishmentExercise, Punishment, PunishmentLog, CardioActivity } from '@/types'

interface PunishmentProgressCardProps {
...
function DonutChart({ progress, size = 80, strokeWidth = 8, color = 'red' }: DonutChartProps) {
  const radius = (size - strokeWidth * 2) / 2
  const circumference = 2 * Math.PI * radius
  const offset = circumference - (progress * circumference)
  
  const colorClasses = {
    red: 'stroke-red-500 opacity-75',
    ...
    <div className="relative inline-flex items-center justify-center">
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        <circle
          ...
          strokeLinecap="round"
          className={colorClasses[color as keyof typeof colorClasses] || colorClasses.red}
...
export default function PunishmentProgressCard({
  progress,
  punishment,
  exercises,
  logs,
  userId,
}: PunishmentProgressCardProps) {
  ...
  useEffect(() => {
    const dateMatch = punishment.end_date.match(/^(\d{4})-(\d{2})-(\d{2})/)
    let endDate: Date
    if (dateMatch) {
      const [, year, month, day] = dateMatch
      endDate = new Date(parseInt(year), parseInt(month) - 1, parseInt(day), 23, 59, 59, 999)
    } else {
      endDate = new Date(punishment.end_date)
  ...
  return (
    <div className="glass-card rounded-2xl soft-shadow-lg p-3 sm:p-5 mb-4 border border-red-300/50 bg-red-50/30">
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-4">
        {/* Cardio Progress */}
        {punishment.cardio_target && punishment.cardio_metric && (
          <div className="flex flex-col items-center">
            ...
            <div className="text-xs sm:text-sm font-medium text-gray-800 mt-1 sm:mt-2 text-center mb-2 sm:mb-3">
              {progress.cardio_total.toFixed(1)} / {punishment.cardio_target} {punishment.cardio_metric === 'miles' ? 'mi' : 'min'}
            </div>
            {/* Cardio Breakdown */}
            <div className="w-full space-y-0.5 sm:space-y-1 text-center">
              {Object.entries(cardioBreakdown).map(([activity, amount]) => (
                <div key={activity} className="text-[10px] sm:text-xs text-gray-600 font-medium">
                  {activity}: {amount.toFixed(1)} {punishment.cardio_metric === 'miles' ? 'mi' : 'min'}
                </div>
              ))}
            </div>
          </div>
